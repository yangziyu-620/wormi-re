...
DATASET_ROOT = Path(
    os.environ.get("WORMI_ALFWORLD_DATA_ROOT", DATA_DISK / "wormi-data" / "alfworld")
)
WORLD_CKPT_ROOT = Path(
    os.environ.get("WORMI_WORLD_ALFWORLD_OUTPUT_DIR", DATA_DISK / "wormi-checkpoints" / "world-alfworld")
)
OUTPUT_DIR = Path(
    os.environ.get("WORMI_ALFWORLD_OUTPUT_DIR", DATA_DISK / "wormi-checkpoints" / "wormi-alfworld")
)
...
trainer_args = WorMITrainerConfig(
    max_steps=int(os.environ.get("WORMI_ALFWORLD_STAGE2_INNER_STEPS", "30")),
    eval_steps=30,
    save_steps=240,
    logging_steps=5,
    batch_size=int(os.environ.get("WORMI_ALFWORLD_STAGE2_BATCH_SIZE", "4")),
    gradient_accumulation_steps=int(
        os.environ.get("WORMI_ALFWORLD_STAGE2_GRADIENT_ACCUMULATION_STEPS", "1")
    ),
    learning_rate=1e-5,
...
curricula = WorMICurricula(
    ...
    meta_learning=True,
    num_iterations=int(os.environ.get("WORMI_ALFWORLD_STAGE2_META_STEPS", "8")),
    world_models=[
        WorldModel(
