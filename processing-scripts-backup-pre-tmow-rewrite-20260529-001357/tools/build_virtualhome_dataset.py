...
from tools.compact_virtualhome_observations import select_task_instances
...
def _class_for_node(graph: dict, node_id: int) -> str:
    node = _nodes_by_id(graph)[int(node_id)]
    name = str(node["class_name"])
    if node.get("category") == "Rooms":
        return _canon_room(name)
    return name
...
def _append_walk(
    ...
    if node_id is None:
        return
    action_text = f"walk {_class_for_node(graph, node_id)}"
    if action_texts and action_texts[-1] == action_text:
        return
...
def _paperlike_program(
    ...
    """
    selection = select_task_instances(graph, family, args)
    ...
    if family in {"turnon", "open"}:
        ...
        target_room = selection.get("target_room")
        ...
        start_room = selection.get("start_room")
        current_room = start_room
        if target_room is not None and target_room != current_room:
    ...
    source_room = selection.get("source_room")
    target_room = selection.get("target_room")
    source_container = selection.get("source_container")

    start_room = selection.get("start_room")
    current_room = start_room
    if source_room is not None and source_room != current_room:
