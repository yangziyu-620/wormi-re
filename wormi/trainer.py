...
class WorMIMetaLearningTrainer(WorMITrainer):
    ...
    def _load_params(self, params: list[torch.Tensor]) -> None:
        for model_param, param in zip(self.model.parameters(), params):
            model_param.data.copy_(param.data.to(model_param.device))
            model_param.requires_grad = True

    def _save_meta_model(self, name: str) -> None:
        output_dir = self.curricula.output_dir / self.curricula.run_name / name
        output_dir.mkdir(parents=True, exist_ok=True)
        self.model.save_pretrained(output_dir)
        self.model.config.save_pretrained(output_dir)
        self.tokenizer.save_pretrained(output_dir)

    def _ensure_adapter_built(self) -> None:
        if self.model.has_model_been_built:
            return
        first_curriculum = self.curricula.train[0]
        first_world = self.curricula.world_models[first_curriculum.world_models[0]]
        aux_model = AutoModelForCausalLM.from_pretrained(
            first_world.model_name, torch_dtype=torch.bfloat16
        )
        self.model.implant(aux_model, first_world.connections)
        self.model.remove_all()
        self.model.release_memory()
        del aux_model
        torch.cuda.empty_cache()

    def _implant_curriculum_world_models(
        self, curriculum: WorMICurriculum
    ) -> None:
        self.model.remove_all()
        self.model.release_memory()
        for target in curriculum.world_models:
            model = self.curricula.world_models[target]
            aux_model = AutoModelForCausalLM.from_pretrained(
                model.model_name, torch_dtype=torch.bfloat16
            )
    ...
    def _build_curriculum_datasets(
        self, curriculum: WorMICurriculum, options: dict
    ...
    def _train_sequential_reptile(self, **options) -> None:
        self.device = self.model.device
        self._ensure_adapter_built()
        meta_params = self._snapshot_params()
        total_inner_steps = 0

        for iteration in range(self.curricula.num_iterations):
            print(
                f"\r🧬 Meta iteration {iteration + 1}/"
                f"{self.curricula.num_iterations}",
                flush=True,
            )
            iteration_start_params = [p.clone() for p in meta_params]
            task_params: list[list[torch.Tensor]] = []

            for task_idx, curriculum in enumerate(self.curricula.train):
                print(
                    f"\r🏃‍➡️ Meta task {task_idx + 1}/"
                    f"{len(self.curricula.train)}: {curriculum.name}",
                    flush=True,
                )
                self._load_params(iteration_start_params)
                self._implant_curriculum_world_models(curriculum)
                train_dataset, test_dataset = self._build_curriculum_datasets(
                    curriculum, options
                )

                trainer_args = deepcopy(curriculum.trainer_args)
                trainer_args.max_steps = curriculum.trainer_args.max_steps
                trainer_args.save_steps = 0
                if curriculum.name is not None and trainer_args.logging_dir is None:
                    trainer_args.logging_dir = (
                        f"iter-{iteration + 1:02d}/{curriculum.name}"
                    )
                ...
                total_inner_steps += curriculum.trainer_args.max_steps
                task_params.append(self._snapshot_params())

                del trainer, train_dataset, test_dataset
                self.model.release_memory()
                torch.cuda.empty_cache()
                gc.collect()

            mean_params: list[torch.Tensor] = []
            for param_idx, old_param in enumerate(iteration_start_params):
                stacked_sum = sum(params[param_idx] for params in task_params)
                mean_param = stacked_sum / len(task_params)
                mean_params.append(
                    old_param
                    + self.curricula.meta_learning_rate
                    * (mean_param - old_param)
                )

            meta_params = mean_params
            self._load_params(meta_params)

            if self.save_steps and total_inner_steps % self.save_steps == 0:
                self._save_meta_model(f"checkpoint-{total_inner_steps}")
                print(
                    f"\r💾 Model saved at step {total_inner_steps}",
                    flush=True,
                )

        self._load_params(meta_params)
        self._save_meta_model("last")
...
class MetaLearningAggregationCallback(
    ...
    def on_iteration_end(
        ...
        **kwargs,
        ...
        use_reptile_beta = os.environ.get(
        ...
        for old_param, main_param in zip(
            ...
            if use_reptile_beta:
                ...
                main_param.data = mean_param.cpu()
