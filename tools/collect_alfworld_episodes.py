"""Collect ALFWorld textual-env expert episodes as room-based source JSONL.

This is the low-level source collector for ALFWorld. The main benchmark
materialization path is now `tools/build_alfworld_dataset.py`, which consumes
the episode JSONL produced here or the archived `share/alfworld-initial-*`
package and emits validated method-comparison releases.

The collector runs the official ALFWorld textual environment on CPU, follows
`extra.expert_plan`, and records one row per solvable episode:

    {task, trial_name, history: [{observation, action, reward, dones, next_observation}, ...]}

Output remains room-based under `<output_dir>/{bathrooms|bedrooms|kitchens|livingrooms}`
so it can be used as a stable source pool for protocol materialization.
"""
